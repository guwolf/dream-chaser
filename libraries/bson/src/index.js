...
function encode(data, schema, endianness, bufferLength) {
  // 判断schema的payload是否已设置length字段，否，则获取payload的长度
  if([null, undefined].includes(schema.payload.length)) {
    const length = codec.String.converter.getStrLength(data.payload);
    data.length = length;
...
function decode(buffer, schema, endianness) {
  const dataView = new DataView(buffer);
  const data = {};
  let offset = 0;
